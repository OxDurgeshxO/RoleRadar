...
import { BulletOptimizer } from "./bullet-optimizer";
import {
  AtsCard,
  InsightSection,
  InterviewPrepCard,
...
export function ResultsView({
  ...
  function generateMarkdown(): string {
    const topRole = result.recommended_roles[0]?.role_name ?? "Top Role";
    const topScore = result.recommended_roles[0]?.match_score ?? 0;
    const date = new Date(analysis.created_at).toLocaleDateString();

    let md = `# RoleFit AI Career Intelligence Report\n\n`;
    md += `**Date:** ${date}  \n`;
    md += `**Top Match Role:** ${topRole} (${topScore}% Match)  \n`;
    md += `**ATS Readiness Score:** ${ats.score}/100  \n\n`;
    md += `## Executive Summary\n\n${result.overall_summary}\n\n`;
    md += `## Top Recommended Roles\n\n`;
    result.recommended_roles.forEach((r, idx) => {
      md += `${idx + 1}. **${r.role_name}** — ${r.match_score}% Match\n`;
    });
    md += `\n## All Scored Roles & Skill Breakdown\n\n`;
    result.role_analysis.forEach((ra) => {
      md += `### ${ra.role_name} (${ra.match_score}%)\n`;
      md += `- **Present Skills:** ${ra.skills_present.join(", ") || "None detected"}\n`;
      md += `- **Partial Matches:** ${ra.skills_partial.join(", ") || "None"}\n`;
      md += `- **Missing Skills:** ${ra.skills_missing.join(", ") || "None"}\n`;
      md += `- **Analysis:** ${ra.short_explanation}\n\n`;
    });
    md += `## Learning Roadmap (${roadmapRole})\n\n`;
    weeks.forEach((w) => {
      md += `### Week ${w.week}: ${w.focus}\n${w.details}\n\n`;
    });
    md += `---\n*Generated by RoleFit — AI Career Intelligence Platform*\n`;
    return md;
  }
  ...
  function downloadMarkdown() {
    ...
    a.href = url;
    a.download = `rolefit-report-${analysis.id.slice(0, 8)}.md`;
    a.click();
    ...
    <div className="mx-auto max-w-6xl px-5 pt-12 pb-24 sm:px-8">
      ...
      <div className="no-print flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {copiedMd ? <Check className="size-3.5 text-brand" strokeWidth={2.4} /> : <FileText className="size-3.5" strokeWidth={2.2} />}
